...
export default {
  ...
  methods: {
	...
	this.ruleForm.xinwenneirong = this.ruleForm.xinwenneirong.replace(reg,'../../../springbootbtw32/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xinwengonggao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xinwengonggaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
